...
async def test_queue_lifecycle_has_consistent_audit_logs(monkeypatch, tmp_path: Path):
    ...
    assert "Unsubscribe" in pending["pending_emails"][0]["body"]
    assert "ChiefAIOfficer.com" in pending["pending_emails"][0]["body"]
    ...
    assert "Unsubscribe" in approved_payload["body"]
