#!/usr/bin/env python3
"""
Run deployed endpoint auth smoke checks across staging + production.
...
from endpoint_auth_smoke import run_smoke


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run endpoint auth smoke checks for staging and production."
    )
    ...
    parser.add_argument(
        "--production-token",
        required=True,
        help="Production dashboard auth token.",
    )
    parser.add_argument(
        "--timeout-seconds",
        type=int,
    ...
    staging = run_smoke(args.staging_url, args.staging_token, timeout_seconds=args.timeout_seconds)
    production = run_smoke(
        args.production_url,
