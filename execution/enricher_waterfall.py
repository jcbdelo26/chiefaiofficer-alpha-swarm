...
Provider Priority (pipeline enrichment):
...
Usage:
    python execution/enricher_waterfall.py --input .hive-mind/scraped/leads.json
    python execution/enricher_waterfall.py --lead-id "uuid" --linkedin-url "url"

Test Mode (no real API calls):
    python execution/enricher_waterfall.py --input .hive-mind/scraped/leads.json --test-mode
...
class WaterfallEnricher:
    """Enrich leads using waterfall enrichment (Apollo -> BetterContact)."""
    # NOTE: Previously named ClayEnricher. Renamed because this module does NOT
    # use Clay's API — it uses Apollo.io + BetterContact waterfall.
    ...
    def __init__(self, test_mode: bool = False):
        ...
        if test_mode:
            ...
            if self.apollo_key:
                ...
                console.print("[dim]Enrichment provider: BetterContact (no Apollo key)[/dim]")
            else:
                console.print("[yellow]No enrichment API key found (APOLLO_API_KEY or BETTERCONTACT_API_KEY). Using mock mode.[/yellow]")
                self.test_mode = True
    ...
    def enrich_lead(self, lead_id: str, linkedin_url: str, name: str = "", company: str = "",
        ...
        2. BetterContact (BETTERCONTACT_API_KEY) — async waterfall (20+ sources)
        3. Mock — test data fallback
        """
...
def main():
    parser = argparse.ArgumentParser(description="Enrich leads via waterfall providers")
...
# Backward-compatibility alias — allows existing `from execution.enricher_waterfall import ClayEnricher`
ClayEnricher = WaterfallEnricher
